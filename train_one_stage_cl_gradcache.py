...
from models.bert.modeling_bert import BertForOneStageCLGradCache
from models.one_stage_cl_grad_cache import GradCache
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.9,
        help="relative ratio between CE and SCL",
    )
    parser.add_argument(
    ...
    assert args.scl_ratio >= 0 and args.scl_ratio < 1
    return args

def eval_model(model, test_dataloader, accelerator, metrics):
    model.eval()
    ...
    with torch.no_grad():
        for _, batch in tqdm(enumerate(test_dataloader), total=len(test_dataloader)):
            ...
            logits = logits[:,0] # choose the original view of data
...
def main():
    ...
    if args.model_name_or_path == 'bert-base-uncased':
        model = BertForOneStageCLGradCache.from_pretrained(
            args.model_name_or_path,
            config=config,
            model_args=args,
        )
    ...
    if args.eval_before_train:
        eval_metrics = eval_model(model, test_dataloader, accelerator, metrics)
        logger.info(f"Eval before train: {eval_metrics}")
    
    running_loss_val = torch.tensor(0.0).to(accelerator.device)
    running_CE_loss_val = torch.tensor(0.0).to(accelerator.device)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            running_CE_loss_val += loss_dict['CE_loss']
            ...
            if (completed_steps-globalstep_last_logged) % args.logging_steps == 0:
                ...
                running_CE_loss_val_scalar = running_CE_loss_val.item()
                ...
                running_CE_loss_val -= running_CE_loss_val
                ...
                log_info['CE_loss'] = round(running_CE_loss_val_scalar / (completed_steps - globalstep_last_logged), 6)
