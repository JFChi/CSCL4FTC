...
""" Finetuning model for sequence classification with no trainer (fix text encoder)"""
...
def parse_args():
    ...
    parser.add_argument(
        "--fine_tune_whole_network",
        action="store_true",
        help="whether fine-tune whole network or not",
    )
    ...
    assert args.model_type is not None and args.model_type in ["bert"]
    return args
...
def main():
    ...
    if args.fine_tune_whole_network:
        logger.info("Fine-tuning the whole network")
        # Optimizer
        ...
        logger.info("Fine-tuning the last layers")

        # disable model require_grad (except pooler and classifier)
        for n, p in model.named_parameters():
            if ('pooler' not in n) and ('classifier' not in n):
                p.requires_grad = False

        # Optimizer
        # Split weights in two groups, one with weight decay and the other not.
        # only add model pooler and cls to optimizer_grouped_parameters
        no_decay = ["bias", "LayerNorm.weight"]
        modules_to_optimize = ["pooler", "classifier"]
        # parameter w/ and w/o weight decay (we only finetune pooler and classifier)
        params_w_wd = [p for n, p in model.named_parameters() if (not any(nd in n for nd in no_decay)) and (any(mo in n for mo in modules_to_optimize))] 
        params_wo_wd = [p for n, p in model.named_parameters() if any(nd in n for nd in no_decay) and (any(mo in n for mo in modules_to_optimize))]
        optimizer_grouped_parameters = [
            {
                "params": params_w_wd,
                "weight_decay": args.weight_decay,
    ...
    for epoch in range(args.num_train_epochs):
        ...
        if args.push_to_hub and epoch < args.num_train_epochs - 1 and args.save_model:
            accelerator.wait_for_everyone()
    ...
    with torch.no_grad():
        ...
        eval_metrics, _ = metrics.compute()
        logger.info(f"Performance on test set after training {epoch} epoch(s): {eval_metrics}")
